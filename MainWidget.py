...
class MainWidget(QWidget):
    ...
    def __InitView(self):
        ...
        self.setWindowTitle("Draw Music")
        ...
        self.__btn_gen_Music = QPushButton("Music")
        self.__btn_gen_Music.setParent(self)
        self.__btn_gen_Music.clicked.connect(self.on_btn_gen_Music_Clicked)
        sub_layout.addWidget(self.__btn_gen_Music)
        ...
        self.__label_penThickness.setText("Width")
    ...
    def on_btn_gen_Music_Clicked(self):
        infile = open('tmp/strokes.pkl','rb')
        notestk = pickle.load(infile)
        infile.close()
        # print(notestk)

        notelen = len(notestk)
        # print(notelen)
        len_pernote = int(notelen/17)
        # print(len_pernote)
        for i in range(17):
            if self.single_note != 0:
                self.note.append(int(self.single_note))
                self.single_note = 0
                print(self.note)
            for l in range(len_pernote):
                if i < 17:
                    self.single_note = self.single_note + notestk[i*len_pernote + l][1]
                    self.single_note = self.single_note/20

        writenote(self.note)
        # pklfile = 'tmp/notes.pkl'
        # outfile = open(pklfile,'wb')
        # pickle.dump(self.note, outfile)
        # outfile.close()
